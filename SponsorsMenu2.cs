using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.SceneManagement;

public class SponsorsMenu2 : MonoBehaviour {

	// Use this for initialization
	void Start () {
		
	}
	
	// Update is called once per frame
	void Update () {
		
	}
    public void b1()
    {
        SceneManager.LoadScene("Sponsors8", LoadSceneMode.Single);
    }
    public void b2()
    {
        SceneManager.LoadScene("Sponsors9", LoadSceneMode.Single);
    }
    public void b3()
    {
        SceneManager.LoadScene("Sponsors10", LoadSceneMode.Single);
    }
    public void b4()
    {
        SceneManager.LoadScene("Sponsors11", LoadSceneMode.Single);
    }
    public void b5()
    {
        SceneManager.LoadScene("Sponsors12", LoadSceneMode.Single);
    }
    public void b6()
    {
        SceneManager.LoadScene("Sponsors13", LoadSceneMode.Single);
    }
    public void b7()
    {
        SceneManager.LoadScene("Sponsors14", LoadSceneMode.Single);
    }
    public void b8()
    {
        SceneManager.LoadScene("Albums", LoadSceneMode.Single);
    }
    public void b9()
    {
        SceneManager.LoadScene("SponsorsMenu3", LoadSceneMode.Single);
    }
    public void b10()
    {
        SceneManager.LoadScene("SponsorsMenu", LoadSceneMode.Single);
    }
}
